...
public sealed class HardwareMonitorService : INotifyPropertyChanged, IDisposable
{
    ...
    private void OnSlowTick()
    {
        ...
        {
            // Always fire — even if sensors returned 0 — so the ViewModel can
            // execute fallback display logic (e.g. iGPU proxy, N/A messaging).
            Notify(nameof(GpuTemperatureC));
            Notify(nameof(GpuUsagePercent));
            Notify(nameof(FanSpeedRpm));
        }
    }
    ...
    // ── Fan sensor scan — walks hardware recursively to find any Fan sensor ────
    ...
    private static void ScanFan(IHardware hw, ref int fanRpm, ref bool gotFan)
    {
        ...
        // Recurse into sub-hardware (e.g. Motherboard → Super I/O → NCT6xxx)
        foreach (var sub in hw.SubHardware)
        {
            ScanFan(sub, ref fanRpm, ref gotFan);
            if (gotFan) return;
        }
    }
